...
def main() -> int:
    ...
    for name, cmd in (("шаблоны", ["tools/check_rules.py"]),
                      ...
                      # ⚠️ Текст соседей по папке модов. Признак ГАСИТ перевод,
                      # поэтому важнее не «ловит ли он чужое», а «не задевает
                      # ли наше»: «Odin» сидит внутри «exploding». Гоняется
                      # по всему живому дампу — там задетых должно быть 0.
                      ("чужие моды", ["tools/check_foreign.py"]),
