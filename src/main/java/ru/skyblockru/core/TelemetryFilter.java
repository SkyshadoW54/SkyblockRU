...
public final class TelemetryFilter {
	...
	// ⚠️ Признак «строка чужого мода» живёт в ForeignMods: его спрашивает
	// не только отправка, но и ПЕРЕВОД со СБОРОМ. Копия признака в этом
	// проекте расходилась трижды, поэтому место у него одно.
