...
public class SkyblockRuClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		ClientPlayConnectionEvents.JOIN.register((handler, sender, client) -> {
			...
			// ⚠️ Отметка «эта установка играла сегодня» — по входу В РЕЖИМ.
			// Без неё нельзя ответить, сколькими людьми мод пользуется:
			// пакеты со строками кончаются через несколько вечеров, и дальше
			// активный игрок неотличим от удалившего мод.
			ru.skyblockru.core.Telemetry.pingOnJoin();
		});
